...
//Get e set pra usar o root fora em outros arquivos ao estilo POO
...
Record search(char *word){
	...
	while(word[i] != '\0'){
		...
		i++;
	}//toupper não funciona para letras com caracteres especiais
	Node *aux = getRoot();
	while(aux != NULL && strcmp(aux->record.word, word) != 0){
		if(strcmp(word, aux->record.word) < 0)
			aux = aux->l;
		else if(strcmp(word, aux->record.word) > 0)
			aux = aux->r;
	}
